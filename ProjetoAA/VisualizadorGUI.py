    ...
    def desenhar_ambiente(self, ambiente, agente):
        ...
        for y in range(self.altura):
            for x in range(self.largura):
                x1, x2 = x * self.tamanho, (x + 1) * self.tamanho
                y1 = (self.altura - 1 - y) * self.tamanho
                y2 = y1 + self.tamanho
                cor = "white"
        ...
        if pos_obj:
            cx, cy = pos_obj.x * self.tamanho, (self.altura - 1 - pos_obj.y) * self.tamanho
        ...
        if pos_agente:
            ax, ay = pos_agente.x * self.tamanho, (self.altura - 1 - pos_agente.y) * self.tamanho
